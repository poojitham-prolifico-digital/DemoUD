function cell(block, r, c = 0) { return block.children[r]?.children?.[c] || null; }
function el(tag, cls, kids=[]) { const e=document.createElement(tag); if(cls) e.className=cls; kids.forEach(k=>e.append(k)); return e; }
...
export default function decorate(block) {
  ...
  ctaA.href = href; ctaA.textContent = label;
  if (ctaIcon) ctaA.append(' ', el('span', `cta__icon ${ctaIcon}`));
  ctaP.append(ctaA);

  // assemble: **exactly two children** in the grid
